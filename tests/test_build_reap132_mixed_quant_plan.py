...
def test_plan_is_deterministic_and_routed_only(tmp_path):
    structural, audit = inputs(tmp_path)
    plan_a, types_a = MODULE.build(structural, audit)
    plan_b, types_b = MODULE.build(structural, audit)
...
def test_rejects_unaccepted_audit(tmp_path):
    structural, audit = inputs(tmp_path, status="INCOMPLETE")
    try:
        MODULE.build(structural, audit)
